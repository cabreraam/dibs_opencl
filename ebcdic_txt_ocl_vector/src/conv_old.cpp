...
int main(int argc, char* argv[])
{
...
#else // not using fpga
#ifdef MWI //create program with source
	char *kernel_source_cpu = getKernelSource("./kernels/e2a_manual_unroll.cl");
	ocl_info.program = clCreateProgramWithSource(ocl_info.context, 1, 
		(const char **) &kernel_source_cpu, NULL, &kernel_status);
#else
	char *kernel_source_cpu = getKernelSource("./kernels/e2a.cl");
	ocl_info.program = clCreateProgramWithSource(ocl_info.context, 1, 
		(const char **) &kernel_source_cpu, NULL, &kernel_status);
#endif //MWI
  ...
  if(status != CL_SUCCESS) 
	...
	unsigned char *dst = (unsigned char *) 
		clSVMAlloc(ocl_info.context, CL_MEM_WRITE_ONLY, sizeof(unsigned char) * newLen,
			1024);
